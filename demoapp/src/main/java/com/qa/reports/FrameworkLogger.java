...
import com.qa.utils.screenshot.ScreenshotUtils;
...
public final class FrameworkLogger {
    ...
    public static void log(LogType status, String message) {
        if(ConfigFactory.getConfig().passedStepsScreenshots().equalsIgnoreCase("yes") && status.equals(LogType.PASS))
            SCREENSHOTMAP.getOrDefault(status, EXTENTANDCONSOLE).accept(message);
        else if(ConfigFactory.getConfig().failedStepsScreenshots().equalsIgnoreCase("yes")&&  status.equals(LogType.FAIL))
            SCREENSHOTMAP.getOrDefault(status, EXTENTANDCONSOLE).accept(message);
        else{
